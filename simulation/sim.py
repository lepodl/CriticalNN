...
def simulate_hetero_model_after_assimilation_for_rest_state(block_path, ip, noise_rate, route_path=None,
    ...
    # TODO(leopold): to ensure that this parametert is a good initializtion.
    # specific_gui = np.array([0.00840336, 0.00028571, 0.05952381, 0.00595238])
    specific_gui = np.array([0.00840336, 0.00028571, 0.07653061, 0.00500756])
    for i, idx in enumerate([10, 11, 12, 13]):
        population_info = np.stack(np.meshgrid(populations, idx, indexing="ij"),
                                   axis=-1).reshape((-1, 2))
        population_info = torch.from_numpy(population_info.astype(np.int64)).cuda()

        alpha = torch.ones(total_populations, device="cuda:0") * specific_gui[i] * 1e8
        beta = torch.ones(total_populations, device="cuda:0") * 1e8
        block_model.gamma_property_by_subblk(population_info, alpha, beta, debug=False)
        ...
        # hp = torch.ones(total_populations, dtype=torch.float32, device="cuda:0") * specific_gui[i]
        # block_model.mul_property_by_subblk(population_info, hp)
