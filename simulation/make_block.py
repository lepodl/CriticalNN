...
def make_small_block(write_path, initial_parameter=(0.00495148, 0.0009899, 0.08417509, 0.00458287)):
    ...
    connect_for_multi_sparse_block(prob, {'g_Li': 0.03,
                                          ...
                                          "V_reset": -65,
                                          'tao_ui': tau_ui},
                                   E_number=int(1.6e3), I_number=int(4e2), degree=100, init_min=1,
...
def make_multi_size_block(write_path, initial_parameter=(0.00408739, 0.00059394, 0.03636364, 0.00353535)):
    ...
    for i in range(rank, total, size):
        ...
        kwords = [{"V_th": -50,
                   ...
                   'T_ref': 5,
                   'tao_ui': (8, 40, 10, 50),
                   "E_number": int(max(b * size_list[i], 0)) if j % 2 == 0 else 0,
                   "I_number": int(max(b * size_list[i], 0)) if j % 2 == 1 else 0, }
...
def make_degree_distribution_block(write_path, initial_parameter=(0.00364106, 0.00074747, 0.02777778, 0.00401235),
    ...
    rank = comm.Get_rank()
    size = comm.Get_size()
    sigma_list = np.arange(5, 90, 10)
    total = sigma_list.shape[0]
    for i in range(rank, total, size):
        current_dir = os.path.join(write_path, f"shifed_exp_sigma_{sigma_list[i]}")
        conn_prob = np.array([[0.8, 0.2],
                              [0.8, 0.2]])
        block_size = np.array([0.8, 0.2])
        degree = np.array([100, 100], dtype=np.uint16)
        kwords = [{"V_th": -50,
                   ...
                   'g_ui': initial_parameter,
                   'tao_ui': (8, 40, 10, 50),
                   "E_number": int(max(b * num_size, 0)) if j % 2 == 0 else 0,
                   "I_number": int(max(b * num_size, 0)) if j % 2 == 1 else 0, }
                  for j, b in enumerate(block_size)]
        conn = connect_for_multi_sparse_block(conn_prob, kwords,
                                              degree=degree,
                                              init_min=1,
                                              ...
                                              perfix=None,
                                              dtype=['single'],
                                              split_EI=True, degree_distribution="shifted_exp", sigma=sigma_list[i], minum_degree=10)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--write_path", type=str, default="../notebook/small_block_d100_same_w")
